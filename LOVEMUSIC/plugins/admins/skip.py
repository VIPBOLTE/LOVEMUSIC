...
from LOVEMUSIC.utils.inline.play import stream_markup, telegram_markup
from LOVEMUSIC.utils.stream.autoclear import auto_clean
from LOVEMUSIC.utils.thumbnails import gen_thumb
...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        try:
            await LOVE.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        try:
            await LOVE.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_7"])
        ...
        try:
            await LOVE.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_7"])
        if videoid == "telegram":
            button = telegram_markup(_, chat_id)
