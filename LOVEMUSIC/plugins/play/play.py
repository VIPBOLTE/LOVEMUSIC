...
from LOVEMUSIC.utils.stream.queue import put_queue, put_queue_index
from youtubesearchpython.__future__ import VideosSearch
from LOVEMUSIC.plugins.bot.shayri import SHAYRI  # Added import for SHAYRI
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                # Getting a random Shayri message
                shayri_msg = SHAYRI[randint(0, len(SHAYRI)-1)]
                # Adding Shayri along with song details
                msg_with_shayri = f"{shayri_msg}\n\n{title}\n"
                # Including original stream string along with Shayri and song details
                msg_with_original_stream = f"{msg}\n{msg_with_shayri}"
                await put_queue(
                    chat_id,
                    original_chat_id,
                    file_path if direct else f"vid_{vidid}",
                    msg_with_original_stream,
                    duration_min,
                    user_name,
                    vidid,
                    user_id,
